/**
 * WordPress Product Cache
 * In-memory cache with TTL for WordPress API responses
 */

const CACHE_TTL = 10 * 60 * 1000; // 10 minutes in milliseconds

class WPProductCache {
  constructor() {
    this.cache = new Map();
  }

  /**
   * Generate cache key from brand ID
   * @param {number} brandId - Brand taxonomy ID
...
/**
 * Get cache instance
 * @returns {WPProductCache}
 */
function getCache() {
  if (!cacheInstance) {
    cacheInstance = new WPProductCache();
